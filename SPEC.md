...
- Offloaded synchronous, blocking disk I/O (`os.scandir` in `discover_models`) to a thread pool using `await asyncio.to_thread()` in FastAPI `async def` endpoints (`/models` and `/chat/start`) to prevent blocking the main asyncio event loop and improve server concurrency.
