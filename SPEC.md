...
- In layout helper functions that dynamically generate a `QLabel` for an input widget (like `_labeled_row` in `settings_panel.py`), tooltips are now programmatically copied from the input widget to the generated label, ensuring users see jargon explanations when hovering over either element.
