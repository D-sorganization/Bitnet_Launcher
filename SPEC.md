...
- Detail labels in the `ModelPanel` and `HubDialog` have been updated with `Qt.TextInteractionFlag.TextSelectableByMouse`, `Qt.TextInteractionFlag.TextSelectableByKeyboard`, and `Qt.TextInteractionFlag.LinksAccessibleByMouse` to allow all users (including keyboard navigators) to select and copy text such as file paths and HuggingFace repo IDs.
