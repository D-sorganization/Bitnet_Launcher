...
## 2026-09-16 - [SECURITY ENHANCEMENT] Prevent Local API Leakage via Referer Header

**Vulnerability:** The local FastAPI server (, , ) did not set the header in HTTP responses. While a local API, missing this header means that if a user follows an external link generated by or present in the local application's UI, the browser could leak the local API URL or application state via the header to the external domain.
**Learning:** Even local APIs should prevent potential information leakage to external domains by explicitly restricting the header using the security header.
**Prevention:** Always explicitly set when configuring security headers for local FastAPI servers in the middleware to ensure no local URLs or state are leaked to external domains.

## 2024-05-24 - [SECURITY ENHANCEMENT] Prevent Local API Leakage via Referer Header

**Vulnerability:** The local FastAPI server (`/chat/start`, `/chat/send`, `/models`) did not set the `Referrer-Policy: no-referrer` header in HTTP responses. While a local API, missing this header means that if a user follows an external link generated by or present in the local application's UI, the browser could leak the local API URL or application state via the `Referer` header to the external domain.
**Learning:** Even local APIs should prevent potential information leakage to external domains by explicitly restricting the `Referer` header using the `Referrer-Policy: no-referrer` security header.
**Prevention:** Always explicitly set `Referrer-Policy: no-referrer` when configuring security headers for local FastAPI servers in the `add_security_headers` middleware to ensure no local URLs or state are leaked to external domains.
