...
## 2026-05-27 - [Prevent DoS via Concurrency Limit and Race Condition Fix in API]

**Vulnerability:** The FastAPI `/chat/start` endpoint did not enforce a maximum number of concurrent `LocalLlamaRunner` instances. Additionally, because `discover_models` blocked the event loop and the runner assignment happened asynchronously, parallel duplicate requests could bypass checks (race conditions), leading to CPU/Memory exhaustion and Denial of Service (DoS) via uncontrolled subprocess spawning.
**Learning:** In async state registries, checking bounds and inserting values must be atomized or explicitly handle pending states to prevent race conditions. Further, long-running processes (like LLM inference) triggered by API endpoints must have explicit concurrency limits, and cleanup should avoid race conditions during `await runner.stop()`.
**Prevention:** 1) Explicitly check a global limit (e.g., `len(active_runners) >= 1`). 2) Immediately insert a placeholder (`None`) in the async registry before `await` operations. Ensure a `try...except BaseException:` block is used to catch `asyncio.CancelledError` if a client disconnects, correctly freeing the lock. 3) Wrap blocking I/O (like `os.scandir` in `discover_models`) using `await asyncio.to_thread()`. 4) In the cleanup finally block, `pop()` the registry entry *after* `awaiting` the resource's `stop` method to prevent OOM overlapping.
