...
## 2025-04-18 - [Prevent SSE Injection in FastAPI StreamingResponses]
**Vulnerability:** Untrusted multiline output (e.g., from an AI model) yielded via a FastAPI `StreamingResponse` was improperly formatted for Server-Sent Events (SSE). Literal newline characters within `chunk` were not prefixed with `data: `, allowing an attacker to inject arbitrary SSE fields (such as `event: ` or `id: `) into the stream, leading to an SSE Injection vulnerability.
**Learning:** SSE relies on double newlines to terminate events and requires every line of payload data to begin with `data: `. When yielding untrusted, multiline content inside `f"data: {chunk}\n\n"`, literal newlines in the chunk allow the payload to break out of the `data: ` block and define other protocol fields.
**Prevention:** Explicitly sanitize newlines in chunked output by replacing them with `\ndata: ` (e.g., `chunk.replace('\n', '\ndata: ')`) before formatting as Server-Sent Events.
