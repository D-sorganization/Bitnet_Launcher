...
## 2024-05-24 - [API Race Condition & Event Loop Blocking]
**Vulnerability:** A race condition existed in the `/chat/start` endpoint where rapid duplicate requests for the same model could bypass the initialization lock, leading to multiple runner processes overlapping and potentially causing OOM errors. Additionally, synchronous `discover_models` calls (which perform blocking disk I/O) were blocking the FastAPI asyncio event loop.
**Learning:** In async state registries where a placeholder (`None`) is used during initialization to prevent race conditions, explicitly check for this pending state (e.g., `if key in registry and registry[key] is None`) to block concurrent duplicate requests. When replacing an active resource, explicitly remove the old resource and set the `None` placeholder *before* `await`ing the old resource's `.stop()` method to prevent concurrent requests from bypassing the initialization lock during the context switch. In FastAPI `async def` endpoints, do not directly call synchronous functions that perform blocking disk I/O.
**Prevention:** Always wrap the initialization code in a `try...except BaseException:` or `finally` block to remove the `None` placeholder if an exception occurs, preventing permanent deadlocks for future requests. When stopping or cleaning up a resource tracked in an async global registry, `pop()` the entry from the dictionary *after* `await`ing its stop method. Use `await asyncio.to_thread()` to offload synchronous blocking disk I/O calls to a thread pool.
