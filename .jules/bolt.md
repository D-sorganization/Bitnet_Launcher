...
## 2024-06-25 - Offload Blocking I/O in FastAPI Endpoints
**Learning:** Calling synchronous blocking functions (like `os.scandir` in `discover_models()`) directly inside a FastAPI `async def` endpoint blocks the entire asyncio event loop, causing severe latency spikes for concurrent API requests.
**Action:** When calling blocking functions from an `async def` endpoint, wrap them with `await asyncio.to_thread()` to offload the I/O operation to a thread pool and keep the event loop non-blocking.
