...
## 2024-05-30 - Python Walrus Operator for Expensive String Operations

**Learning:** In Python generator expressions, list comprehensions, or `if` conditions inside loops, evaluating expensive string operations (like `.lower()`) multiple times on the same object (e.g., checking `.endswith()` and `in` separately) is redundant and allocates unnecessary memory per iteration.
**Action:** Use the walrus operator (`:=`) to assign the result of the expensive string operation to a local variable within the expression (e.g., `(fname := f.name.lower()).endswith(".gguf") and "tq2_0" in fname`). This avoids redundant method calls and string allocations per iteration, improving performance.
