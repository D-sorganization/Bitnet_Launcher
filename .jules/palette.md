...
## 2025-05-30 - Programmatic Tooltip Copying for Labels

**Learning:** In PyQt6 layout helper functions that dynamically generate a `QLabel` for an input widget, users often hover over the label itself to understand the input field, especially for technical settings like hyperparameters. If the tooltip is only applied to the input field, this valuable context is missed.
**Action:** When dynamically wrapping input widgets with labels, programmatically copy the input's tooltip to the label (e.g., `if widget.toolTip(): lbl.setToolTip(widget.toolTip())`) so both elements display necessary jargon explanations on hover, improving discoverability.
