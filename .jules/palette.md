...
## 2023-11-09 - Copying Jargon Tooltips to Labels

**Learning:** When form fields use domain-specific jargon (like "Context size"), users might hover over the label expecting an explanation, especially if the input field is small. If the tooltip is only on the input widget, discoverability suffers.
**Action:** In PyQt6 layout helper functions that dynamically generate a `QLabel` for an input widget, programmatically copy the input's tooltip to the label (e.g., `if widget.toolTip(): lbl.setToolTip(widget.toolTip())`) so both elements display the necessary explanation on hover.
