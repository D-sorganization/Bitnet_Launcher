...
## 2026-07-15 - Dynamic Label Tooltip Propagation

**Learning:** When using layout helper functions to generate form rows with technical or jargon-heavy hyperparameters (like "Threads" or "Context size" in `_labeled_row`), assigning a tooltip only to the input widget leaves the label unhelpful.
**Action:** When dynamically generating labels for inputs in helper functions, check if the input widget has a tooltip (`if widget.toolTip():`) and copy it to the `QLabel` so users get explanations when hovering over either element.
