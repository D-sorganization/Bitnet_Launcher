...
## 2024-07-04 - Settings Jargon Tooltips

**Learning:** When form fields use domain-specific jargon or technical hyperparameters (like "Context size" or "Temperature"), users often hover over the label rather than the input field itself to seek clarification. If tooltips are only attached to the input field, users might miss important explanatory text.
**Action:** In layout helpers that dynamically generate a `QLabel` for an input widget, programmatically copy the input's tooltip to the label (e.g., `if widget.toolTip(): lbl.setToolTip(widget.toolTip())`) so both elements display the necessary explanation on hover.
