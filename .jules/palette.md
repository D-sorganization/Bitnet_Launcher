...
## 2024-05-24 - Dynamically Propagating Tooltips to Labels

**Learning:** When using layout helper functions to dynamically generate an associated `QLabel` for an input widget in PyQt6, if the input widget has a descriptive tooltip (e.g., explaining domain-specific jargon), the label will lack this context unless explicitly copied.
**Action:** In PyQt6 layout helper functions that generate labels for input widgets, always programmatically copy the input's tooltip to the label (e.g., `if widget.toolTip(): lbl.setToolTip(widget.toolTip())`) so both elements display necessary jargon explanations on hover.
