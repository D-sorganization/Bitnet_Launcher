...
async def start_chat(request: ChatStartRequest) -> StreamingResponse:
    ...
    if request.model_name in active_runners:
        existing = active_runners[request.model_name]
        if existing is None:
            raise HTTPException(status_code=400, detail="Initialization in progress")

        # Immediately set placeholder so parallel requests hit the 400 error
